...
class PaymentsService {
  ...
  public async getPayementsAuth(req: RequestWithUser): Promise<any> {
    ...
    if(req.user.id !== customerKey) throw new HttpException(401, "유저 인증에 실페했습니다")
    const authData = await tossClient('POST', '/v1/brandpay/authorizations/access-token', {
      grantType: 'AuthorizationCode',
      code,
      customerKey
    })
    await userModel.updateOne({id: req.user.id}, {$set: {toss_accessToken: authData.data.accessToken, toss_refreshToken: authData.data.refreshToken, toss_tokenType: authData.data.tokenType}})
    if(authData.error) throw new HttpException(401, authData.message ? authData.message : "유저 인증에 실페했습니다")
    return authData
  }

  public async getPaymentsMethods(req: RequestWithUser): Promise<any> {
    let tossMethodsData = await tossClient('GET', '/v1/brandpay/payments/methods', null, req.user.toss_accessToken as string)
    if(tossMethodsData.data.code === "INVALID_ACCESS_TOKEN") {
      const refreshToken = await tossRefreshToken(req.user)
      if(!refreshToken) throw new HttpException(401, "유저 인증에 실페했습니다")
      tossMethodsData = await tossClient('GET', '/v1/brandpay/payments/methods', null, refreshToken.accessToken as string)
    }
    if(tossMethodsData.error) throw new HttpException(401, tossMethodsData.message ? tossMethodsData.message : "유저 인증에 실페했습니다")
  ...
  public async confirmPayment(req: RequestWithUser): Promise<any> {
    const {amount, methodId, orderId, paymentKey, email, phone} = req.body as confirmPayment
  ...
  public async getSuccessOrder(req: RequestWithUser): Promise<any> {
    const payments = await paymentsModel.findOne({orderId: req.params.orderId})
    if(!payments || req.user.id !== payments.userId) throw new HttpException(404, "해당 결제는 찾을 수 없습니다")
    let itemMetadata
    let nextPayDate: Date
    if(payments.type === "guild") {
      const guild = client.guilds.cache.get(payments.target)
      if(!guild) throw new HttpException(404, "결제를 진행했던 서버를 찾을 수 없습니다")
      itemMetadata = {
        type: "guild",
        id: guild.id,
        icon: guild.icon,
      ...
      const guildPremium = await premiumGuildModel.findOne({guild_id: guild.id})
      nextPayDate = guildPremium.nextpay_date
    } else if (payments.type === "user") {
      const user = client.users.cache.get(payments.target)
      if(!user) throw new HttpException(404, "결제를 진행했던 유저를 찾을 수 없습니다")
      itemMetadata = {
        type: "user",
        id: user.id,
        avatar: user.avatar,
  ...
  public async getOrder(req: RequestWithUser): Promise<any> {
    const payments = await paymentsModel.findOne({orderId: req.params.orderId})
    if(!payments || payments.userId !== req.user.id) throw new HttpException(404, "해당 결제는 찾을 수 없습니다")
    if(payments.process === "success") throw new HttpException(400, "해당 결제는 이미 완료되었습니다")
    let itemMetadata
    if(payments.type === "guild") {
      const guild = client.guilds.cache.get(payments.target)
      if(!guild) throw new HttpException(404, "구매를 진행하는 서버를 찾을 수 없습니다")
      itemMetadata = {
        type: "guild",
        id: guild.id,
        icon: guild.icon,
      ...
      const user = client.users.cache.get(payments.target)
      if(!user) throw new HttpException(404, "구매를 진행하는 유저를 찾을 수 없습니다")
      itemMetadata = {
        type: "user",
        id: user.id,
        avatar: user.avatar,
